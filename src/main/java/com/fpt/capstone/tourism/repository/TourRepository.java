...
public interface TourRepository  extends JpaRepository<Tour, Long>, JpaSpecificationExecutor<Tour> {
    ...
    @Query("SELECT t FROM Tour t WHERE t.deleted = FALSE AND t.open = TRUE ORDER BY t.createdAt DESC LIMIT 1")
    Tour findNewestTour();

    @Query("")
    Integer findAvailableSeats(Long tourId);
}
